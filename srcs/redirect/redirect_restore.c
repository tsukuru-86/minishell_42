...
static void	restore_fds(t_redirect *last_out, t_redirect *last_in)
{
	...
	{
		debug_print_with_int("[DEBUG] restore_fds: Restoring STDOUT from fd: ",
		...
		debug_print_with_int("[DEBUG] restore_fds: Closed original_fd for STDOUT: ",
			last_out->original_fd);
		last_out->original_fd = -1;
	}
	if (last_in && last_in->original_fd != -1)
	{
		debug_print_with_int("[DEBUG] restore_fds: Restoring STDIN from fd: ",
			last_in->original_fd);
		if (dup2(last_in->original_fd, STDIN_FILENO) == -1)
			perror("minishell: dup2 STDIN_FILENO restore");
		close(last_in->original_fd);
		debug_print_with_int("[DEBUG] restore_fds: Closed original_fd for STDIN: ",
			last_in->original_fd);
...
void	cleanup_heredocs(t_redirect *redirect)
{
	...
	while (current)
	{
		...
		{
			debug_print_with_str("[DEBUG] cleanup_heredocs: Unlinking heredoc file: ",
				current->file);
...
void	restore_redirection(t_redirect *redirect)
{
	...
	if (!redirect)
	{
		debug_print("[DEBUG] restore_redirection: No redirects to restore,
			returning.");
		return ;
	}
