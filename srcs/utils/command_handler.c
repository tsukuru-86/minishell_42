...
int	handle_tokens_and_parse(t_token *tokens)
{
	...
	{
		debug_print("[DEBUG] handle_tokens_and_parse: Empty command detected,
			returning 0.");
		return (0);
	}
	...
	if (!cmd)
	{
		debug_print("[DEBUG] handle_tokens_and_parse: Command parsing failed,
			returning 2.");
		return (2);
	}
	...
	debug_print_with_int("[DEBUG] handle_tokens_and_parse: After process_commands,
		status: ", status);
