...
int	setup_pipeline(t_command *cmd)
{
	...
	if (!cmd->next)
	{
		debug_print("[DEBUG] setup_pipeline: Single command, no pipeline needed, returning 1.");
		return (1);
	}
	...
	if (!create_pipes(cmd))
	{
		...
		debug_print("[DEBUG] setup_pipeline: Failed to create pipes, returning 0.");
		return (0);
	}
	...
	if (!spawn_pipeline_processes(cmd))
	{
		...
		debug_print("[DEBUG] setup_pipeline: Failed to spawn processes, cleaning up and returning 0.");
...
int	execute_command_pipeline(t_command *cmd)
{
	...
	if (!cmd)
	{
		debug_print("[DEBUG] execute_command_pipeline: Command is NULL, returning 1.");
		return (1);
	}
	debug_print("[DEBUG] execute_command_pipeline: Before setup_pipeline.");
	pipeline_result = setup_pipeline(cmd);
	debug_print_with_int("[DEBUG] execute_command_pipeline: After setup_pipeline, result: ", pipeline_result);
	if (pipeline_result == 0)
	{
		ft_putstr_fd((char *)"minishell: pipeline setup error\n", 2);
		debug_print("[DEBUG] execute_command_pipeline: Pipeline setup error, cleaning up and returning 1.");
		cleanup_pipeline_list_all(cmd);
		return (1);
	}
	debug_print("[DEBUG] execute_command_pipeline: Before wait_pipeline.");
	status = wait_pipeline(cmd);
	debug_print_with_int("[DEBUG] execute_command_pipeline: After wait_pipeline, status: ", status);
	...
	cleanup_pipeline_list_all(cmd);
	debug_print_with_int("[DEBUG] execute_command_pipeline: Exit with status: ", status);
	return (status);
}
