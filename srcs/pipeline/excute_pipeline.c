...
/*   Updated: 2025/07/01 23:05:21 by muiida           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	validate_pipeline(t_command *cmd)
{
	...
	{
		debug_print(
			"[DEBUG] setup_pipeline: Single command, no pipeline needed, returning 1.");
		return (1);
	}
	return (2);
}
...
static int	setup_pipes_and_processes(t_command *cmd)
{
	if (!create_pipes(cmd))
	{
		...
		debug_print(
			"[DEBUG] setup_pipeline: Failed to create pipes, returning 0.");
		return (0);
	}
	debug_print(
		"[DEBUG] setup_pipeline: After create_pipes. Before spawn_pipeline_processes.");
	if (!spawn_pipeline_processes(cmd))
	{
		ft_putstr_fd((char *)"minishell: failed to spawn processes\n", 2);
		debug_print(
			"[DEBUG] setup_pipeline: Failed to spawn processes, cleaning up and returning 0.");
...
int	setup_pipeline(t_command *cmd)
{
	...
	if (!setup_pipes_and_processes(cmd))
		return (0);
	debug_print(
		"[DEBUG] setup_pipeline: After spawn_pipeline_processes. Before close_parent_pipes.");
	close_parent_pipes(cmd);
	debug_print(
		"[DEBUG] setup_pipeline: After close_parent_pipes. Returning 1.");
	return (1);
}
...
int	execute_command_pipeline(t_command *cmd)
{
	...
	{
	debug_print(
		"[DEBUG] execute_command_pipeline: Command is NULL, returning 1.");
	return (1);
}
	debug_print("[DEBUG] execute_command_pipeline: Before setup_pipeline.");
	pipeline_result = setup_pipeline(cmd);
	debug_print_with_int(
		"[DEBUG] execute_command_pipeline: After setup_pipeline, result: ",
		pipeline_result);
	if (pipeline_result == 0)
	{
		ft_putstr_fd((char *)"minishell: pipeline setup error\n", 2);
		debug_print(
			"[DEBUG] execute_command_pipeline: Pipeline setup error, cleaning up and returning 1.");
	...
	debug_print_with_int(
		"[DEBUG] execute_command_pipeline: After wait_pipeline, status: ",
		status);
	if (cmd && cmd->redirects)
	{
		debug_print(
			"[DEBUG] execute_command_pipeline: Cleaning up heredocs.");
		cleanup_heredocs(cmd->redirects);
	}
	debug_print(
		"[DEBUG] execute_command_pipeline: Cleaning up pipeline list all.");
	cleanup_pipeline_list_all(cmd);
	debug_print_with_int(
		"[DEBUG] execute_command_pipeline: Exit with status: ", status);
	return (status);
}
